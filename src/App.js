    ...
    <div className="App">
      ...
      <header className="App-header">
        <div>
          ...
          <button disabled={task.length < 0} className='button-dugme' onClick={editmode? handleupdate:klik}>{editmode?'UPDATE':'CREATE'}</button>
        </div>
        <div>
          {toDo.map((doo,index)=>(
            <div key={index} className='todo-wraper'>
              <p className={`${doo.isCompleted ?'complete':''}`}>{doo.name}</p>
              <div className='action-wrapper'>
                <button className='complete-button' onClick={()=>handleComplete(index)}disabled={doo.isCompleted}>COMPLETED</button>
                <button className='edit-button' onClick={()=>handleedit(doo,index)}disabled={doo.isCompleted}>EDIT</button>
                <button className='dugme' onClick={()=>deletee(index)}>DELETE</button>
              </div>
            </div>
          ))}
        </div>
